describe("Truck Management API Tests", () => {
    ...
    it("should not create a new Truck without Name", () => {
        ...
        cy.request({method: "POST", url:`${apiUrl}trucks`, body:sampleTruckNoName, failOnStatusCode: false}).then((response) => {
    ...
    it("should create a new Truck", () => {
      cy.request("POST", `${apiUrl}trucks`, sampleTruck).then((response) => {
        ...
        console.log(sampleTruckId);
        cy.log(sampleTruckId);
    ...
    //     cy.request(
    //         {method: "PUT", url:`${apiUrl}trucks/${sampleTruckId}`, status:"TO_JOB", failOnStatusCode: false}  
    //     ).then((response) => {
    //       expect(response.status).to.equal(400);
    ...
    //   cy.request(
    //     {
    //     method: "PUT", url:`${apiUrl}trucks/${sampleTruckId}`,  status: "InvalidStatus", failOnStatusCode: false
    //     ,
    //   }).then((response) => {
    //     expect(response.status).to.equal(400);
    //   });
    // });
      ...
      it("should handle invalid status when creating a Truck", () => {
        ...
        cy.request({method: "POST", url:`${apiUrl}trucks`,  truckWithInvalidStatus, failOnStatusCode: false}
      ...
      it("should handle duplicate Truck code when creating a Truck", () => {
        cy.request({method: "POST", url:`${apiUrl}trucks`,  sampleTruck, failOnStatusCode: false}
        ).then((response) => {
          expect(response.status).to.equal(400);
        });
      });
    ...
    //     const updatedStatus = "AT_JOB";
    //     cy.request(
    //         {method: "PUT", url:`${apiUrl}trucks/999999`, {status: updatedStatus}, {failOnStatusCode:false},
    //           },).then((response) => {
    //       expect(response.status).to.equal(404);
    //     });
    //   });
    
    //   it("should not delete a non-existent Truck", () => {
    //     const nonExistentTruckCode = "NonExistentCode";
    //     cy.request("DELETE", `${apiUrl}trucks/${nonExistentTruckCode}`).then((response) => {
    //       expect(response.status).to.equal(404);
    //     });
    //   });
